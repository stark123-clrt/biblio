...
try {
    ...
    if (strlen($text) > 2000) {
        ...
        $text = substr($text, 0, 2000); // Limiter à 2000 caractères
    }
    ...
    // ✅ CONFIGURATION PIPER
    $voiceModel = 'fr_FR-tom-medium.onnx'; // Voix Tom Medium
    
    // Paramètres optimisés pour la qualité et la vitesse
    $length_scale = 1.0;  // Vitesse normale
    $noise_scale = 0.4;   // Expression modérée
    $noise_w = 0.4;       // Intonation stable
    ...
    $maxWaitTime = 20; // 20 secondes maximum
    $waitCount = 0;
    $checkInterval = 0.5; // Vérifier toutes les 500ms
    ...
    while ($waitCount < $maxWaitTime) {
        if (file_exists($fullPath)) {
            ...
            if ($currentSize > 1000) {
                // Attendre encore un peu pour s'assurer que l'écriture est terminée
                usleep(500000); // 500ms
                
                $finalSize = filesize($fullPath);
                
                // Vérifier que la taille est stable
        ...
        usleep($checkInterval * 1000000); // Convertir en microsecondes
    ...
    if (isset($fullPath) && file_exists($fullPath) && filesize($fullPath) < 1000) {
        unlink($fullPath); // Supprimer fichier corrompu
    }
    ...
    // ✅ GESTION DES ERREURS FATALES PHP
